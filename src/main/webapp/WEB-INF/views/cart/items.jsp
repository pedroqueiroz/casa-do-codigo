...
<tags:pageTemplate title="Seu carrinho de compra">
				...
				<c:forEach items="${shoppingCart.items}" var="item">
					<tr>
						...
						<td class="quantity-input-cell">
                            <input type="number" min="0" id="quantity" name="quantity" value="${shoppingCart.getQuantity(item)}" />
						</td>
						<td class="numeric-cell">${shoppingCart.getTotalPrice(item)}</td>
						<td class="remove-item">
							<form:form action="${s:mvcUrl('SCC#removeItem').arg(0, item.product.id).arg(1, item.priceType).build()}" method="POST">
